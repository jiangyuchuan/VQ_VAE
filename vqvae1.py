...
# 
# As an example for a `BCHW` tensor of shape `[16, 64, 32, 32]`, we will first convert it to an `BHWC` tensor of shape `[16, 32, 32, 64]` 
# and then reshape it into `[16384, 64]` and all `16384` vectors of size `64`  will be quantized independently. In otherwords, the channels are used as the space in which to quantize. All other dimensions will be flattened and be seen as different examples to quantize, `16384` in this case
class VectorQuantizer(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # Calculate distances/encoder输出与codebook
...
for i in range(10):
    ...
    for i in xrange(num_training_updates):
        ...
        if (i+1) % 100 == 0:
            ...
            print()
    #输出model._vq_vae._embedding.weight.data到codingbook_list[]中
    codingbook_list.append(model._vq_vae._embedding.weight.data)
...
valid_reconstructions = (valid_reconstructions + 1) / 2
from torchvision.utils import save_image

# 可调整放置格式，连同存储路径以及文件名一起构成path
path = './valid_reconstructions.png'
...
# 可调整放置格式，连同存储路径以及文件名一起构成path
path = './valid_source.png'

# 使用save_image()函数来将张量保存为图像文件
save_image(valid_reconstructions, path)

# ## View Embedding
#对codingbook_list[]中的每个codingbook进行umap降维
import umap
proj = umap.UMAP(n_neighbors=3,
                    min_dist=0.1,
                    metric='cosine').fit_transform(codingbook_list[0].cpu())
# torch.save(model._vq_vae._embedding.weight.data.cpu(), 'tensor.pt')
